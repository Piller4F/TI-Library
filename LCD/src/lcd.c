...
@author   杜帅
@version  V1.0
@introduction 默认的扫描方向   从左到右,从上到下 
             分辨率320*240
             在mini开发板上靠右插
             引脚： LCD_LED ->PC10    LCD_CS->PC9    LCD _RS->PC8    LCD _WR->PC7    LCD _RD->PC6    LCD _D[17:1]->PB[15:0]; 
             每个引脚含义：
             LCD_CS->TFTLCD 片选信号          LCD _RS->命令/数据标志（0，读写命令；1，读写数据） 
             LCD _WR->向 TFTLCD 写入数据      LCD _RD->从 TFTLCD 读取数据    LCD _D->16 位双向数据线
常用功能函数
显示数字,高位为0,则不显示   void LCD_ShowNum(u16 x,u16 y,u32 num,u8 len,u8 size)
                x,y :起点坐标	 x:0-280  y:0-320
				len :数字的位数    
				size:字体大小    
				color:颜色     
				num:数值(0~4294967295);	 
显示数字,高位为0,还是显示   void LCD_ShowNum(u16 x,u16 y,u32 num,u8 len,u8 size)
				x,y:起点坐标
				num:数值(0~999999999);	 
				len:长度(即要显示的位数)
				size:字体大小
				...
				[6:1]:保留
				[0]:0,非叠加显示;1,叠加显示.
显示字符串void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p)
				x,y:起点坐标
				width,height:区域大小  
				size:字体大小
				*p:字符串起始地址
显示字符函数void LCD_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode)
				x,y:起始坐标
				num:要显示的字符:" "--->"~"
				size:字体大小 12/16/24
				mode:叠加方式(1)还是非叠加方式(0)
画圆函数    void LCD_Draw_Circle(u16 x0,u16 y0,u8 r)                x0,y0中心点   r半径
画矩形	    void LCD_DrawRectangle(u16 x1, u16 y1, u16 x2, u16 y2)  (x1,y1),(x2,y2):矩形的对角坐标
画线        void LCD_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2)       x1,y1:起点坐标    x2,y2:终点坐标  
